            ...
            return (
              <Link key={tab.href} href={tab.href} className="relative -mt-6" onClick={playHapticClick}>
                <motion.div
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="w-14 h-14 rounded-full bg-gradient-to-br from-[#00C8FF] to-[#FF1133] flex items-center justify-center text-black shadow-md shadow-[#00C8FF]/15 border border-white/10 active:shadow-inner cursor-pointer"
                >
                  <Icon size={22} className="stroke-[2.5]" />

                  {/* Pulsing outer ring (más sutil) */}
                  <div className="absolute inset-0 rounded-full border border-[#00C8FF]/40 animate-ping opacity-15 pointer-events-none" />
                </motion.div>
